...
#import "FTRootNavifationController.h"

@implementation FTRootNavifationController
- (id)init {
  self = [self init];
  if (self) {
    // 只少2个 头一个肯定是顶级的界面
...
- (void)pushViewController:(UIViewController *)viewController
  ...
  if (animated) {
    ...
    if ([[self viewControllers] count] >
        0) { //作为rootViewController的时候不用animated
...
- (UIViewController *)popViewControllerAnimated:(BOOL)animated {
  ...
  if (animated) {
    ...
    [self addLastScreenShotView];
    //先缩放 kScaleValue
    lastScreenShotView.transform =
        CGAffineTransformMakeScale(kScaleValue, kScaleValue);
